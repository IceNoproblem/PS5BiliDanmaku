...
特别说明：本人完全是非专业开发者，无任何编程和开源相关经验，纯属闲得无聊、凭着对PS5和B站弹幕的热爱，瞎折腾出了这个小工具。开发全程依赖AI工具辅助（从代码编写到文档整理，几乎没有自己独立完成的专业操作），由于对开发规范、开源规则完全不熟悉，若项目中存在僭越任何规则、潜规则，或是不符合专业标准的地方，还请各位技术大佬多多海涵，也非常欢迎大家提出修改建议，帮我完善这个小工具。
...
1. 先确认部署机器已安装 Git（未安装的新手，直接放弃此方式，选择方式一！安装教程：https://git-scm.com/downloads）

2. 打开部署机器的命令行（Windows 用 CMD/PowerShell，Linux/macOS 用终端）

3. 打开你的 GitHub 项目仓库，找到「Clone HTTPS」链接（仓库页面绿色「Code」按钮，点击后复制链接）

4. 在命令行中，先粘贴第一步复制的克隆链接，按下回车（示例：git clone https://github.com/你的GitHub用户名/PS5BiliDanmaku.git，无需手动修改，直接复制你仓库的链接即可）

5. 克隆完成后，粘贴以下命令，按下回车，进入项目文件夹：
...
git clone https://github.com/你的GitHub用户名/PS5BiliDanmaku.git
cd PS5BiliDanmaku
